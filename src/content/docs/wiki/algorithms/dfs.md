...
```moonmaid
flowchart TD { A["開始ノードを選択"] -> B["訪問済みにする"] B -> C{"未訪問の隣接ノードがある？"} C -> |"Yes"| D["隣接ノードへ進む"] D -> B C -> |"No"| E["一つ前に戻る（バックトラック）"] E -> C }
```
...
```moonmaid
flowchart TD { A["グリッド全走査"] -> B{"grid[i][j] == '1' ?"} B -> |"Yes"| C["count++ → DFS 開始"] B -> |"No"| A C -> D["現セルを '0' に塗り替え"] D -> E["上下左右に再帰"] E -> A }
```
