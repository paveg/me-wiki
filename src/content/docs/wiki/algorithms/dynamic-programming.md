...
```moonmaid
flowchart TD { A["問題を受け取る"] -> B{"最適部分構造がある?"} B -> |"No"| Z["DP は不適"] B -> |"Yes"| C{"重複部分問題がある?"} C -> |"No"| D["分割統治で十分"] C -> |"Yes"| E["DP を適用"] E -> F{"状態の依存関係が単純?"} F -> |"Yes"| G["Bottom-up で実装"] F -> |"No"| H["Top-down（メモ化）で実装"] G -> I["空間最適化を検討"] H -> I }
```
...
$n$ = 入力サイズ（配列長、目標金額など）、$m$ = 第2次元（コインの種類数、グリッドの列数、文字列長など）
