...
```moonmaid
flowchart TD { A["配列を左から右に走査"] -> B["現在の要素に関する情報を HashMap で参照"] B -> C{"条件を満たすか?"} C -> |"Yes"| D["結果を記録"] C -> |"No"| E["HashMap を更新"] D -> E E -> F{"次の要素があるか?"} F -> |"Yes"| A F -> |"No"| G["結果を返す"] }
```
...
```moonmaid
flowchart TD { A["入力: eat, tea, tan, ate, nat, bat"] -> B["各文字列をソート"] B -> C["aet → eat, tea, ate"] B -> D["ant → tan, nat"] B -> E["abt → bat"] C -> F["グループ化して返す"] D -> F E -> F }
```
...
```moonmaid
flowchart TD { A["prefix = 0, count = 0"] -> B["HashMap: {0: 1}"] B -> C["配列を走査"] C -> D["prefix += nums[i]"] D -> E{"prefix - k が HashMap にあるか?"} E -> |"Yes"| F["count += HashMap[prefix - k]"] E -> |"No"| G["HashMap[prefix]++"] F -> G G -> C }
```
