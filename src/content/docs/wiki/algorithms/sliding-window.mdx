...
<TabItem label="MoonBit">
```moonbit
for right = 0; right < arr.length(); right = right + 1 {
  // add arr[right] to window state
  if right >= k {
    // remove arr[right - k] from window state
  }
...
function findAnagrams(s: string, p: string): number[] {
  ...
  if (sCount.every((v, i) => v === pCount[i])) result.push(0);
  for (let i = p.length; i < s.length; i++) {
    sCount[s.charCodeAt(i) - a]++;
    sCount[s.charCodeAt(i - p.length) - a]--;
    if (sCount.every((v, j) => v === pCount[j])) {
...
<TabItem label="MoonBit">
```moonbit
fn find_anagrams(s : String, p : String) -> Array[Int] {
  if s.length() < p.length() {
    return []
  }
  let result : Array[Int] = []
  let s_count = Array::make(26, 0)
  let p_count = Array::make(26, 0)
  for i = 0; i < p.length(); i = i + 1 {
    p_count[p[i].to_int() - 'a'.to_int()] += 1
    s_count[s[i].to_int() - 'a'.to_int()] += 1
  }
  if s_count == p_count {
    result.push(0)
  }
  for i = p.length(); i < s.length(); i = i + 1 {
    s_count[s[i].to_int() - 'a'.to_int()] += 1
    s_count[s[i - p.length()].to_int() - 'a'.to_int()] -= 1
    if s_count == p_count {
      result.push(i - p.length() + 1)
    }
  }
...
**ポイント:** Go では固定長配列 `[26]int` の比較が `==` で可能。Python もリスト同士の `==` で比較できる。TypeScript は `every` で要素ごとの比較が必要。
