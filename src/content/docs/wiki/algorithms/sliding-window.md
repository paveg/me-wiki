...
```moonmaid
flowchart TD { A["pCount = p の文字頻度を計算"] -> B["sCount = s の先頭 len(p) 文字の頻度を計算"] B -> C{"sCount == pCount ?"} C -> |"Yes"| D["結果に index を追加"] C -> |"No"| E["ウィンドウを右に 1 スライド"] D -> E E -> F["右端の文字を追加、左端の文字を削除"] F -> C }
```
