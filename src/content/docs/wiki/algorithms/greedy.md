...
```moonmaid
flowchart TD { A["問題を受け取る"] -> B["候補を評価基準でソート/整理"] B -> C{"候補が残っている?"} C -> |"Yes"| D["局所的に最適な候補を選択"] D -> E{"制約を満たす?"} E -> |"Yes"| F["解に追加"] E -> |"No"| G["スキップ"] F -> C G -> C C -> |"No"| H["解を返す"] }
```
...
```moonmaid
flowchart TD { A["区間を終了位置でソート"] -> B["arrows = 1, xEnd = points[0][1]"] B -> C{"次の区間がある?"} C -> |"Yes"| D{"points[i][0] > xEnd ?"} D -> |"Yes"| E["arrows++, xEnd = points[i][1]"] D -> |"No"| F["現在の矢でカバー済み"] E -> C F -> C C -> |"No"| G["arrows を返す"] }
```
