...
```moonmaid
flowchart TD { A["配列の各要素を走査"] -> B{"スタックが空？"} B -> |"Yes"| E["要素をスタックに push"] B -> |"No"| C{"現在の要素 > スタックのトップ？"} C -> |"Yes"| D["トップを pop し答えを記録"] D -> B C -> |"No"| E E -> F{"次の要素がある？"} F -> |"Yes"| A F -> |"No"| G["スタックに残った要素は答えなし"] }
```
