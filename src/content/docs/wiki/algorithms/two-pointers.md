...
```moonmaid
flowchart TD { A["2つのポインタを初期位置にセット"] -> B{"終了条件を満たすか?"} B -> |"No"| C["ポインタの位置から状態を評価"] C -> D{"条件に基づきどちらを動かすか判定"} D -> |"左ポインタ"| E["left を移動"] D -> |"右ポインタ"| F["right を移動"] D -> |"両方"| G["両方を移動"] E -> B F -> B G -> B B -> |"Yes"| H["結果を返す"] }
```
