...
```moonmaid
flowchart TD { I1["Insert: 新しい要素を末尾に追加"] -> I2{"親 > 子？"} I2 -> |"Yes"| I3["親と交換 (bubble up)"] I3 -> I2 I2 -> |"No"| I4["Insert 完了"] E1["Extract: 根を取り出す"] -> E2["末尾の要素を根に移動"] E2 -> E3{"親 > 子？"} E3 -> |"Yes"| E4["最小の子と交換 (bubble down)"] E4 -> E3 E3 -> |"No"| E5["Extract 完了"] }
```
...
```moonmaid
flowchart TD { S["各リストの先頭を min-heap に追加"] -> L{"heap が空？"} L -> |"No"| P["最小ノードを Pop"] P -> A["結果リストに追加"] A -> N{"node.Next != nil？"} N -> |"Yes"| PU["node.Next を Push"] PU -> L N -> |"No"| L L -> |"Yes"| D["完了"] }
```
...
```moonmaid
flowchart LR { L1["lo (max-heap): 5"] -> L2["3"] L2 -> L3["1"] R1["hi (min-heap): 8"] -> R2["10"] R2 -> R3["12"] L1 -> |"lo の最大 ≤ hi の最小"| R1 M["中央値 = (5 + 8) / 2 = 6.5"] }
```
