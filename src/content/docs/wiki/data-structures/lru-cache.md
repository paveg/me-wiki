...
```moonmaid
flowchart LR { HM["HashMap key → Node*"] H["Head (sentinel)"] -> N1["Node A key=1, val=10"] N1 -> H N1 -> N2["Node B key=2, val=20"] N2 -> N1 N2 -> N3["Node C key=3, val=30"] N3 -> N2 N3 -> T["Tail (sentinel)"] T -> N3 HM -> N1 HM -> N2 HM -> N3 }
```
