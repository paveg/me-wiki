...
flowchart TD
    subgraph Get
        G1["Get(key)"] --> G2{"cache に key がある？"}
        G2 -->|Yes| G3["ノードをリストから remove"]
        G3 --> G4["Head の直後に insertHead"]
        G4 --> G5["val を返す"]
        G2 -->|No| G6["-1 を返す"]
    end

    subgraph Put
        P1["Put(key, val)"] --> P2{"cache に key がある？"}
        P2 -->|Yes| P3["val を更新"]
        P3 --> P4["remove → insertHead"]
        P2 -->|No| P5["新しい Node を作成"]
        P5 --> P6["cache に追加 + insertHead"]
        P6 --> P7{"容量を超えた？"}
        P7 -->|Yes| P8["Tail.prev を remove\ncache から delete"]
        P7 -->|No| P9["完了"]
    end
```
